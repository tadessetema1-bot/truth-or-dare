...
/* NEW: prevents Truth/Dare sound from
   playing repeatedly on Firebase updates */
let lastChoiceSoundKey = null;
...
async function playChatUnlockSound() {
    ...
    try {
        ...
        if (context.state === "running") {
            ...
            if (!chatSoundUnlocked) {
                ...
                console.log(
                    "🔊 Chat notification sound unlocked."
                );

                await playChatUnlockSound();
            }
...
async function playChatNotification() {
    ...
    try {
        ...
        if (context.state !== "running") {
            console.log(
                "Cannot play notification. Audio state:",
                context.state
            );
            return;
        }
        ...
        oscillator2.start(now + 0.10);
        oscillator2.stop(now + 0.32);

        console.log(
            "🔔 Chat notification played."
        );

    } catch (error) {

        console.log(
            "Chat notification sound unavailable:",
            error
        );
...
/* =========================
   TRUTH / DARE CHOICE SOUND
========================= */

async function playChoiceSound(type) {
...
doneBtn.addEventListener(
    ...
    async () => {
        ...
        /* Reset sound key so the next
           question can trigger normally */
        lastChoiceSoundKey =
            null;
    ...
    onValue(
        ...
        async (snapshot) => {
            ...
            if (
                ...
                const newestMessageData =
                    newestMessage[1];

                if (
                    newestMessageData.playerId !==
                    currentPlayerId
                ) {
